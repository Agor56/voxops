...
const Hero = () => {
  ...
  const features = [
    { icon: Bot, label: t.hero.features.leadResponse },
    { icon: Calendar, label: t.hero.features.autoScheduling },
    { icon: MessageSquare, label: t.hero.features.whatsapp },
  ];
  ...
  return (
    <section className="relative min-h-screen flex items-center pt-20 overflow-hidden">
      ...
      <div className={`hero-glow bg-secondary top-1/3 ${isRTL ? '-left-48' : '-right-48'} animate-pulse-glow`} style={{ animationDelay: '1.5s' }} />
        ...
        <div className="max-w-4xl mx-auto text-center">
          ...
          >
            {features.map((feature, index) => (
              <div
                key={index}
                className={`glass-card px-5 py-3 rounded-full flex items-center gap-3 hover:border-primary/30 transition-all ${isRTL ? 'flex-row-reverse' : ''}`}
              >
