...
const Header = () => {
  ...
  return <>
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-background/80 backdrop-blur-xl border-b border-border/50 py-3' : 'bg-transparent py-5'}`} dir={isRTL ? 'rtl' : 'ltr'}>
        <div className={`container mx-auto flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
          ...
          <div className={`hidden md:flex items-center gap-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
            ...
            <ThemeToggle />
            <Button variant="hero" size="sm" onClick={() => setIsDemoDialogOpen(true)}>
              {t.nav.bookDemo}
            </Button>
          </div>
        ...
        <AnimatePresence>
          ...
          height: 0
              ...
              <div className="container mx-auto py-4 flex flex-col gap-4">
                {navItems.map(item => <a key={item.label} href={item.href} onClick={e => handleNavClick(e, item)} className="text-foreground py-2">
                    {item.label}
                  </a>)}
