...
const PricingSection = () => {
  ...
  const toggleAddon = (planId: string) => {
    setExpandedAddons(prev => ({ ...prev, [planId]: !prev[planId] }));
  };
  ...
  return (
    <section id="pricing" className="relative py-24 md:py-32 overflow-hidden" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <div className="container mx-auto px-4 relative z-10">
          ...
          {plans.map((plan, index) => (
            <motion.div
              ...
              className={`relative rounded-2xl p-6 flex flex-col ${
                plan.popular
                  ? 'bg-gradient-to-br from-primary to-secondary text-primary-foreground shadow-[0_0_40px_hsl(262_83%_58%/0.4)]'
                  : plan.premium
            ...
            >
              {/* Popular Badge */}
              {plan.popular && (
                <div className={`absolute -top-3 ${isRTL ? 'left-4' : 'right-4'} bg-yellow-400 text-purple-900 px-3 py-1 rounded-full text-sm font-bold flex items-center gap-1`}>
              ...
              <div className={`text-4xl font-bold mb-1 ${plan.popular ? 'text-primary-foreground' : ''}`}>
                {plan.price}
                {plan.period && (
                  <span className="text-lg font-normal opacity-75">/{plan.period}</span>
                )}
              </div>
              {plan.setup && (
                <div className={`text-sm mb-4 ${plan.popular ? 'opacity-75' : 'text-muted-foreground'}`}>
                  {plan.setup}
                </div>
              )}
              {plan.priceNote && (
              ...
              {/* Badge */}
              <div className={`inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-sm mb-4 w-fit ${
                plan.popular
                ...
                <div className="mb-4">
                  <button
                    ...
                    className={`flex items-center gap-2 text-sm cursor-pointer transition-colors ${
                      plan.popular ? 'opacity-75 hover:opacity-100' : 'text-muted-foreground hover:text-foreground'
                    } ${isRTL ? 'flex-row-reverse' : ''}`}
                  >
                    <ChevronDown className={`w-4 h-4 transition-transform ${expandedAddons[plan.id] ? 'rotate-180' : ''}`} />
                    {t.pricing.optionalAddons}
                  </button>
                  {expandedAddons[plan.id] && (
                    <ul className={`text-sm mt-2 space-y-1 ${isRTL ? 'pr-4' : 'pl-4'}`}>
                      {plan.addons.map((addon, i) => (
                        <li key={i} className={`${plan.popular ? 'text-primary-foreground/70' : 'text-muted-foreground'}`}>
                          □ {addon}
                        </li>
                      ))}
                    </ul>
        ...
        >
          <a href="#comparison" className="text-primary hover:text-primary/80 transition-colors font-medium">
            {t.pricing.comparisonLink}
          </a>
        </motion.div>
      </div>
    </section>
