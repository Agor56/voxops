...
const DemoForm = ({ onSubmitted, submissionCount }: DemoFormProps) => {
  ...
  const validate = (): boolean => {
    ...
    if (!phone.trim()) {
      ...
      errors.phone = 'מספר לא תקין (05X-XXXXXXX)';
    }
  ...
  return (
    <div className="space-y-12" dir="rtl">
      ...
      <motion.div
        ...
        className="glass-card p-8 md:p-10 rounded-2xl max-w-2xl mx-auto space-y-6"
      >
        {/* Full Name */}
          ...
          <div className="flex flex-wrap gap-2">
            {BUSINESS_TYPES.map((type) => (
              <button
                key={type}
                type="button"
                onClick={() => { setBusinessType(type); setFieldErrors(p => ({ ...p, businessType: '' })); }}
          ...
          <div className="flex flex-wrap gap-2">
            {TIMELINE_OPTIONS.map((opt) => (
              <button
                key={opt}
                type="button"
                onClick={() => { setTimeline(opt); setFieldErrors(p => ({ ...p, timeline: '' })); }}
                className={`px-4 py-2 rounded-xl border-2 transition-all text-sm font-medium ${
                  timeline === opt
                    ? 'border-primary bg-primary/10 text-primary'
                    : 'border-border/50 hover:border-primary/50 text-foreground'
